...
const MyService = (props) => {
    ...
    const backendURL = (props && props.data && props.data.backendServiceURL) || "";
    ...
    useEffect(() => {
        const fetchOrders = async () => {
            try {
                ...
                const url = backendURL + '/laundrino/myservice?cartid=' + myParam;
                let order_res = await axios.get(url);
                let order = {};
                if(order_res.data.success === true){
                  order = order_res.data && order_res.data.billingDetails || "";
                }
                // let order = {
    ...
    if (error) {
        return <div style={{textAlign:"center",fontSize:"50px"}}>{error}</div>;
    }
