...
const Checkout = (props) => {
  ...
  let isShippingDetailsSame = true;
  ...
  const handleCheckout = async () => {
    try {
      ...
      if (isShippingDetailsSame) {
        ...
        req_data.shippingDetails = shippingDetailsData;
      } else {
        let shippingDetailsData = {
          address: req_data.address || "",
          state: req_data.state || "",
      ...
      const response = await axios.post(url, req_data);
      console.log(response.data);

      if (checkoutHandler && response.data?.success === true && !isCod) {
        checkoutHandler(response);
      } else if (isCod && response.data?.success === true) {
        window.location.href = "/thankyou?cardid=" + myParam;
      }
    } catch (error) {
  ...
  const checkoutHandler = async (props) => {
    try {
      ...
      const options = {
        ...
        order_id: order.id,
        callback_url: serviceBackendURL + "/thankyou?cardid=" + myParam,
        notes: {
      ...
      <div className="m-5">
        ...
        <div className="row">
          ...
          <div className="col-md-8 order-md-1 billDetail">
            ...
            <form className="needs-validation" id="addrValidation" noValidate>
              <div className="row">
                ...
                {/* <div className="col-md-6 mb-3">
                  ...
                  <input type="text" className="form-control" id="lastName" placeholder="" value={formData.lastName} onChange={handleChange} required/>
                  <div className="invalid-feedback">
