...
const Cart = (props) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(
          serviceBackendURL + `/laundrino/get-card-details?id=` + cartId
        );
    ...
    if (cartId && serviceBackendURL) {
      fetchData();
    }
  }, [serviceBackendURL, cartId]);

  const handleDeleteService = (productIndex, serviceKey) => {
    setCartDetails((prevDetails) => {
      const updatedCartDetails = { ...prevDetails };
      const selectedProduct = updatedCartDetails.selectedProduct[productIndex];

      selectedProduct.serviceTotalPrice -=
        selectedProduct.selectedServiceWithPriceValue[serviceKey];
  ...
  const handleDeleteProduct = (productIndex) => {
    setCartDetails((prevDetails) => {
      ...
      let servicePrice = updatedCartDetails.selectedProduct.reduce(
        (total, product) => total + product.serviceTotalPrice,
        0
      );
      ...
      updatedCartDetails.subTotal =
        updatedCartDetails.serviceTotal + updatedCartDetails.shippingCharge;
        ...
        .then((response) => {
          let data_details =
            (response &&
              ...
              response.data.getCartDetails) ||
            "";
          if (data_details.serviceTotal === 0) {
  ...
  const update_cart_data = () => {
    const data = JSON.stringify(cartDetails);

    axios
      .put(serviceBackendURL + "/laundrino/get-card-details?id=" + cartId, data)
      .then(() => {
  ...
  return (
    <>
      {cartDetails && cartDetails.selectedProduct && (
        <div className="container">
                ...
                <div className="card-body cart-summary">
                  <div className="summary-item">
                    ...
                    <div className="summary-value">
                      ₹{cartDetails.serviceTotal}
                    </div>
                  </div>
                  <div className="summary-item">
                    <div className="summary-title">Shipping</div>
                    <div className="summary-value">
                      ₹{cartDetails.shippingCharge}
                    </div>
                  </div>
                  <div className="summary-item">
                  ...
                  <div className="summary-item">
                    ...
                    <div className="summary-value">
                      ₹{Math.round(cartDetails.grandTotal)}
                    </div>
                  </div>
                  <button
                    onClick={update_cart_data}
          ...
          <div className="">
            ...
            {cartDetails.selectedProduct.map((product, productIndex) => (
                    ...
                    <div className="card-body">
                      <div className="d-flex justify-content-between">
                        <p>
                          {product.productName}{" "}
                          <font color="grey">
                            ({product.noOfServiceSelected} Service Selected)
                          </font>
                        </p>
                      </div>
                      <div className="mt-3">
                        {Object.entries(
                          product.selectedServiceWithPriceValue
                          ...
                          <div key={idx}>
                            <div className="service-row d-flex justify-content-between">
                              ...
                              {/* <i
                                ...
                                onClick={() =>
                                  handleDeleteService(productIndex, service)
                                }
                              ></i> */}
                            </div>
